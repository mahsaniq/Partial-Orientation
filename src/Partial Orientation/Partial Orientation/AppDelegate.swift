...
class AppDelegate: UIResponder, UIApplicationDelegate {
    ...
    // Partial Support for Interface Orientation
    func application(_ application: UIApplication, supportedInterfaceOrientationsFor window: UIWindow?) -> UIInterfaceOrientationMask {

        let interfaceOrientationMask: UIInterfaceOrientationMask = .portrait

        if let topViewController = self.topViewControllerWithRootViewController(window?.rootViewController) {

            if topViewController.responds(to: #selector(InterfaceOrientationAllButUpsideDown.supportingInterfaceOrientationAllButUpsideDown)) {
